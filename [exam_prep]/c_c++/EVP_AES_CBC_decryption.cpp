...
int main(int argc, char* argv[]) {
    ...
    cipherKeyLength = EVP_CIPHER_CTX_key_length(context);
    cipherIvLength = EVP_CIPHER_CTX_iv_length(context);
    ...
    fscanf(ft, "%u\n", &plaintext_length);
    unsigned char infile_buffer[INPUT_BUFFER_SIZE];
    ...
    if (!EVP_DecryptUpdate(context, outBuf + outBytes, &tmpOutBytes, buf2crypt, bytesInBuf))
    ...
    if (!EVP_DecryptFinal_ex(context, outBuf + outBytes, &tmpOutBytes))
    ...
    if (!EVP_DecryptUpdate(context, outBuf + outBytes, &tmpOutBytes, buf2crypt, infile_length))
        prtErrAndExit(1, (char*)"ERROR: EVP_DecryptUpdate didn't work...\n");
    outBytes += tmpOutBytes;
